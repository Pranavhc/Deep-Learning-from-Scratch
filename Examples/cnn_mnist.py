...
from nn.layers import Conv2D, Dense, Flatten
...
# very small model considering the runtime being CPU
clf = NeuralNetwork(Adam(), CCE(), [
    Conv2D(in_chnls=n_channels, out_chnls=3, kernel_size=3, stride=2),  # (1, 28, 28) -> (3, 13, 13) 
    ReLu(),
    Conv2D(in_chnls=3, out_chnls=6, kernel_size=3, stride=2),           # (3, 13, 13) -> (6, 6, 6)
    ReLu(),
    Flatten(),                                                          # (6, 6, 6)  -> (6*6*6)
    Dense(6*6*6, 10),
    Softmax()
...
################# SAVE THE MODEL
save_object(clf, 'Examples/models/mnist_clf_cnn.pkl')

################# LOAD THE MODEL
clf = load_object('Examples/models/mnist_clf_cnn.pkl')
